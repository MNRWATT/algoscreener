  ...
  const { data: stock, isLoading } = useQuery<StockDetail>({
    ...
    queryFn: async () => {
      const res = await apiRequest(
        "GET",
        `/api/stock/${ticker}?momentum=30&quality=25&lowVol=20&valuation=10&erm=10&insider=5`
      );
      return res.json();
    },
    enabled: !!ticker,
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="text-xl font-bold tabular-nums">{priceText}</div>
              <div
                className={`text-sm tabular-nums font-medium ${changeColor}`}
              >
                {changeText}
              </div>
            </div>
            <div className="text-center px-3 py-1.5 rounded-lg bg-muted/50 border border-border">
              ...
              <div
                className={`text-2xl font-bold tabular-nums ${compositeColor}`}
              >
                {stock.composite}
              </div>
            </div>
              ...
              return (
                <div key={group.label} className="space-y-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs tabular-nums font-bold ml-auto">
                      {group.score}
                    </span>
                  </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left text-[10px] font-semibold uppercase tracking-wider px-2 py-2">
                      Ticker
                    </th>
                    {FACTOR_CONFIG.map((f) => (
                      <th
                        key={f.key}
                        className="text-center text-[10px] font-semibold uppercase tracking-wider px-2 py-2"
                      >
                        {f.label.slice(0, 3)}
                      </th>
                    ))}
                    <th className="text-center text-[10px] font-semibold uppercase tracking-wider px-2 py-2">
                      Score
                    </th>
                  </tr>
                </thead>
