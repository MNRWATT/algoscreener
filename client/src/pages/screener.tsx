...
function StockRow({
  ...
  const changeColor = stock.change1d == null ? "text-muted-foreground" : stock.change1d >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-red-600 dark:text-red-400";
  ...
  return (
    <>
      ...
      >
        ...
        <td className="px-3 py-2 text-right text-sm tabular-nums font-medium cursor-pointer" onClick={onToggle}>${stock.price != null ? stock.price.toLocaleString() : '—'}</td>
        ...
        <td className="px-3 py-2 text-right text-xs tabular-nums text-muted-foreground hidden md:table-cell cursor-pointer" onClick={onToggle}>
          ${stock.marketCap != null ? `${stock.marketCap != null ? `${stock.marketCap.toFixed(1)}B` : '—'` : '—'}
        </td>
        {FACTORS.map((f) => {
