...
function getMarketRegime(): MarketRegime {
  const vix = 25.75; const yieldSpread10y2y = 0.56; const sp500Ytd = -2.28;
  const recessionProb = 27; const inflationRate = 2.4; const gdpGrowth = 1.4;
...
function seedDemoAlerts() {
  ...
  if (top) alerts.push({ id: randomUUID(), type: "score_above", ticker: top.ticker, threshold: 70, message: `${top.ticker} composite score reached ${top.composite} (threshold: 70)`, createdAt: new Date(Date.now() - 7200000).toISOString(), read: false });
}
seedDemoAlerts();
